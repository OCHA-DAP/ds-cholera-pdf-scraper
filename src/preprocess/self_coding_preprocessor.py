        ...
        for attempt in range(1, max_iters + 1):
            ...
            if result_file.exists():
                try:
                    ...
                    print(f"✅ Successfully extracted {len(data)} records via self-coded preprocessor")
                    
                    # Log successful scripts for debugging and reuse
                    pdf_name = Path(pdf_path).stem
                    script_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    
                    script_log_path = script_logs_dir / f"{pdf_name}_{script_timestamp}_main.py"
                    ...
                    metadata_path = script_logs_dir / f"{pdf_name}_{script_timestamp}_metadata.json"
