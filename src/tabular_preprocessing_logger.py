...
class TabularPreprocessingLogger:
    ...
    def log_tabular_preprocessing(
        ...
        countries_detected = (
            surveillance_df["Country"].nunique()
            if "Country" in surveillance_df.columns
            else 0
        )
        events_detected = (
            surveillance_df["Event"].nunique()
            if "Event" in surveillance_df.columns
            else 0
        )
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            metadata_path = (
                f"outputs/preprocessing/metadata/metadata_{preprocessing_id}.json"
            )
        ...
        print(
            f"   📊 {records_extracted} records, {countries_detected} countries, {events_detected} events"
        )
