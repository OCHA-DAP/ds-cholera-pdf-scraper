...
## Batch Run Analysis: GPT vs Rule-Based Extraction
...
# Load raw data
print("Loading batch extraction results...")
batch_df, list_batch_df = load_batch_data(
    str(repo_root / "outputs" / "batch_run")
)

print("\nLoading rule-based scraper data...")
ruleb_df = pd.read_csv(repo_root / "data" / "final_data_for_powerbi_with_kpi.csv", low_memory=False)
...
print(f"  Filtering baseline to {len(batch_weeks)} year-weeks from batch extraction...")
ruleb_df = ruleb_df.merge(batch_weeks, on=['Year', 'WeekNumber'], how='inner')
print(f"✓ Loaded {len(ruleb_df)} rule-based records (filtered to batch weeks)")

# Perform analysis WITHOUT post-processing corrections
print("\nAnalyzing batch vs rule-based (raw LLM output)...")
...
print(f"\n✓ Analysis complete: {len(disc_cat)} discrepancies categorized")
```

### CFR Consistency Validation

To systematically assess which system is more accurate, we check **CFR (Case Fatality Rate) consistency**: whether `Deaths / TotalCases * 100` matches the stated CFR value.
...
**Key Finding**: LLM has better CFR consistency in **80.6% of discrepancies** (58 of 72 cases with CFR data). This provides strong evidence that the LLM extraction is more accurate than the rule-based baseline.

### Timeline Visualizations of Discrepancies
...
### A2. Discrepancy Distribution by Week
...
### A4. Comma/Thousands Issue Examples
...
### A7. Gap-Filling Corrections (Experimental)
