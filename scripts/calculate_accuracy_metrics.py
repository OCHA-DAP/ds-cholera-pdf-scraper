...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate accuracy metrics for LLM extraction"
    )
    parser.add_argument(
        "--prompt-version",
        "-v",
        type=str,
        help="Prompt version to analyze (e.g., v1.1.0)",
    )
    parser.add_argument(
        "--outputs-dir",
        "-o",
    ...
    parser.add_argument(
        ...
        "-c",
        action="store_true",
        help="Use current prompt version from PromptManager",
    )
    ...
    try:
        ...
        metrics_file = (
            Path(args.outputs_dir) / f"accuracy_metrics_prompt_{prompt_version}.json"
        )
        import json
